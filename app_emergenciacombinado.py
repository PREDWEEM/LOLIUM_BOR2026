...
if df is not None and modelo_ann is not None:
    ...
    # --- B. PREDICCIÓN NEURAL CON DESFASE TEMPORAL ---
    # Aplicamos desfase de +60 días para ajustar la estacionalidad de Bordenave
    ...
    # Sigmoide centrada en 15mm
    df["Hydric_Factor"] = 1 / (1 + np.exp(-0.4 * (df["Prec_sum_21d"] - 15)))
    df["EMERREL"] = df["EMERREL"] * df["Hydric_Factor"]
    
    # Relajación dinámica: Para Bordenave se permite emergencia temprana 
    # si la lluvia > 50mm, eliminando el bloqueo estacional.
    ...
    # VISUALIZACIÓN
    # -----------------------------------------------------
    st.title("🌾 PREDWEEM BORDENAVE - 2026")
    ...
    fig_risk.update_layout(height=120, margin=dict(t=30, b=0, l=10, r=10), title="Mapa de Intensidad de Emergencia (Ajustado Bordenave)")
    ...
    with tab1:
        ...
        with col_main:
            ...
            fig_emer.update_layout(title="Dinámica de Emergencia (Shift +60d)", height=350)
    ...
    st.info("👋 Bienvenido. Cargue datos meteorológicos para comenzar el análisis de Bordenave.")
